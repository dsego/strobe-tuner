...
The strobing effect is generated by a phase comparator algorithm that runs two successive single-bin DFTs tuned to the target note frequency. The phase difference between these DFTs is visualized as a moving strobe pattern. To ensure a consistent strobing response across the frequency range, the hop size and window lengths are adaptively set based on pitch intervals in cents rather than Hz. The strobe sensitivity (i.e. speed) can be adjusted by scaling the detected phase difference.
